...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    @Query("SELECT p FROM Project p JOIN FETCH p.users u WHERE u.id = :userId AND p.id = :projectId")
    Optional<Project> findByUserIdAndProjectId(Long userId, Long projectId);

    @Query("SELECT p FROM Project p JOIN FETCH p.users u WHERE u.id = :userId")
